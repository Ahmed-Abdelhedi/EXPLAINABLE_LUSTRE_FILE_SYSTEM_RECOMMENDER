#!/usr/bin/env python3
"""
plausibility_error_analyzer.py
...
def predicted_label(scenario: Dict[str, Any]) -> str:
    if scenario.get("technical_error") not in (None, "", False):
        return "__TECHNICAL_ERROR__"

    return normalize_upper(
        scenario.get("actual", {}).get("predicted_label")
    )
...
def actual_issues(
    ...
    issues = [
        item
        for item in as_list(
            scenario.get("actual", {}).get("issues")
        )
        if isinstance(item, dict)
    ]
...
def analyze_scenario(
    ...
    technical_error = scenario.get("technical_error")

    if technical_error not in (None, "", False):
        add_error(
            errors,
            scenario,
            "technical_error",
    ...
    exp_label = expected_label(scenario)
    act_label = predicted_label(scenario)
    ...
    exp_block = expected_should_block(scenario)
    act_block = actual_should_block(scenario)
    ...
    for kind in ("blocking", "warning"):
        exp_issues = expected_issues(scenario, kind)
        act_issues = actual_issues(scenario, kind)
        ...
        for issue in missing:
            add_error(
                ...
                {
                    "expected_code": expected_code(issue),
                    "expected_fields": expected_fields(issue),
                },
            )
        ...
        for issue in extras:
            add_error(
                ...
                {
                    "actual_code": actual_code(issue),
                    "actual_field": issue.get("field"),
                    "actual_referenced_fields": actual_referenced_fields(issue),
                },
            )

        for exp_issue, act_issue in matched:
            exp_fields = set(expected_fields(exp_issue))
            act_fields = set(actual_referenced_fields(act_issue))

            missing_fields = sorted(exp_fields - act_fields)

            if missing_fields:
                add_error(
                    errors,
                    scenario,
                    "missing_expected_issue_field_reference",
                    {
                        "issue_code": expected_code(exp_issue),
                        "expected_fields": sorted(exp_fields),
                        "actual_referenced_fields": sorted(act_fields),
    ...
    if bool(actual.get("input_mutated", False)):
        add_error(
            ...
            "input_mutation",
            {
                "input_after_analysis": actual.get(
...
def compute_error_report(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "evaluation_scope": "AI Plausibility dedicated benchmark",
        "summary": {
...
def print_report(report: Dict[str, Any]) -> None:
    ...
    print()
    print("=" * 72)
    print("AI PLAUSIBILITY - ERROR ANALYZER")
    print("=" * 72)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Chemin vers results_ai_plausibility.json",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Chemin vers errors_ai_plausibility.json",
    )
