...
class RequirementChatbot:
    ...
    def _is_explicit_update(self, text: str) -> bool:
        """
        Détecte une intention explicite de remplacement d'une ancienne valeur.

        Cette détection repose sur des familles de formulations et non sur une
        seule forme infinitive. Elle reconnaît notamment :

        - « mets à jour la puissance à 20 kW » ;
        - « modifie le budget » ;
        - « finalement sans HA » ;
        - « remplace 15 kW par 20 kW » ;
        - « update power to 20 kW » ;
        - « set budget to 100000 USD ».

        Une simple nouvelle valeur sans marqueur de modification reste traitée
        normalement et peut donc créer un conflit si elle contredit l'état.
        """
        ...
        update_patterns = (
            # Changement d'avis ou remplacement implicite.
            r"\bfinalement\b",
            r"\bplut[oô]t\b",
            r"\bau\s+lieu\s+de\b",
            r"\b[àa]\s+la\s+place\b",
            r"\bje\s+pr[eé]f[eè]re\b",

            # Mettre à jour et formes conjuguées.
            r"\b(?:mettre|mets?|mettons|mettez|mis|mise|mises|"
            r"mettent)\s+[àa]\s+jour\b",
            r"\bmise\s+[àa]\s+jour\b",

            # Modifier.
            r"\bmodifi(?:er|e|es|ez|ons|ent|[eé]e?s?)\b",

            # Changer.
            r"\bchang(?:er|e|es|ez|ons|ent|[eé]e?s?)\b",

            # Corriger.
            r"\bcorrig(?:er|e|es|ez|ons|ent|[eé]e?s?)\b",

            # Remplacer.
            r"\bremplac(?:er|e|es|ez|ons|ent|[eé]e?s?)\b",

            # Ajuster / réviser.
            r"\bajust(?:er|e|es|ez|ons|ent|[eé]e?s?)\b",
            r"\br[eé]vis(?:er|e|es|ez|ons|ent|[eé]e?s?)\b",

            # Variation quantitative explicite.
            r"\baugment(?:er|e|es|ez|ons|ent|[eé]e?s?)\b",
            r"\br[eé]dui(?:re|s|t|sons|sez|sent)\b",
            r"\bbaiss(?:er|e|es|ez|ons|ent|[eé]e?s?)\b",
    ...
    def _is_rich_message(self, text: str) -> bool:
        """
        Distingue une réponse courte d'un message contenant plusieurs champs.

        Cette détection est structurelle :
        - plusieurs nombres ;
        - plusieurs séparateurs ;
        - plusieurs familles de champs ;
        - phrase suffisamment longue.

        Une unité seule, comme « 15 kW », ne suffit pas à classer le message
        comme riche. Elle peut être une réponse courte à une question active.
        """

        normalized = text.lower().strip()

        number_count = len(
            re.findall(r"-?\d+(?:[\.,]\d+)?", normalized)
        )
        ...
        field_patterns = [
            r"\b(?:tib|tb|tébioctets?|terabytes?)\b",
            r"\b(?:clients?|n(?:oe|œ)uds?|nodes?|compute nodes?)\b",
            r"\b(?:fichiers?|files?|millions?|milliards?)\b",
            r"\b(?:ratio|r\s*[:/]\s*w|r\s*/\s*w)\b",
            r"\b(?:lecture|read)\b",
            r"\b(?:écriture|ecriture|write)\b",
            r"\b(?:budget|usd|dollars?)\b|\$",
            r"\b(?:puissance|power|watts?|kilowatts?|kw)\b",
            r"\b(?:croissance|growth|pourcent|percent)\b",
            r"\b(?:ha|haute disponibilité|haute disponibilite|"
            r"high availability|fault tolerance)\b",
            r"\b(?:accès|acces|access|mixed|mixte|random|"
            r"sequential|séquentiel|parallel|parallèle|streaming)\b",
        ]

        field_family_count = sum(
            1
    ...
    def _apply_ai_plausibility_report_to_state(
        ...
        self.state.calculation_result = {"ready": False}
    ...
    def _try_clarification(self, user_text: str) -> bool:
        """
        Résout uniquement les réponses courtes selon la question active.

        Principe général :
        - réponse courte : interprétation dans le contexte du champ demandé ;
        - message riche : retour vers le pipeline normal afin que tous les
          champs présents soient extraits pendant le même tour.

        Cette séparation évite d'associer le premier nombre d'un message riche
        au mauvais champ et empêche la perte des autres informations du tour.
        """
        ...
        # Une mise à jour explicite doit être traitée par l'extracteur complet.
        if active_issue.candidates and self._is_explicit_update(user_text):
            return False

        # Un message multi-champs ne doit jamais être forcé dans le type du
        # premier champ en attente. process_user_message poursuivra avec
        # HybridExtractor et fusionnera toutes les valeurs extraites.
        ...
        if direct_candidate is not None:
            field_name = direct_candidate.field.value

            self._remove_candidates_for_fields({field_name})
