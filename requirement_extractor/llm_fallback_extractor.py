...
class LLMFallbackExtractor:
    ...
    def _scale_multiplier(
        ...
        """
        Détecte un suffixe d'échelle pour ``total_file_count``.

        Le multiplicateur est seulement une indication de l'échelle exprimée
        dans la réponse du LLM ou dans l'evidence. Son application effective
        est décidée par ``_normalize_total_file_count`` afin d'éviter une
        multiplication en double.

        Exemples reconnus :
        - 5 million / 5 millions / 5M ;
        - 2.5 billion / 2,5 milliards / 2.5B ;
        - 100 thousand / 100 mille / 100k.
        """
        ...
        joined = f"{value_text} {unit_text} {evidence_text}".lower()
        compact_unit = unit_text.strip().lower()

        if (
            re.search(r"\b(?:milliards?|billions?)\b", joined)
            or compact_unit in {"b", "bn", "billion", "milliard"}
            or re.search(r"\d\s*(?:b|bn)\b", joined)
        ):
            return 1_000_000_000

        if (
            re.search(r"\bmillions?\b", joined)
            or compact_unit in {"m", "mn", "million"}
            or re.search(r"\d\s*m\b", joined)
        ):
            return 1_000_000

        if (
            re.search(r"\b(?:mille|thousand)\b", joined)
            or compact_unit in {"k", "thousand", "mille"}
            or re.search(r"\d\s*k\b", joined)
        ):
            return 1_000

        return 1

    def _normalize_total_file_count(
        self,
        value: Any,
        unit: Any,
        evidence: str,
        ...
        """
        Normalise un nombre total de fichiers sans appliquer deux fois
        un multiplicateur déjà intégré par le LLM.

        Deux formes de sortie LLM sont acceptées :

        1. valeur non développée :
           ``value=5, unit="million"`` -> ``5_000_000``

        2. valeur déjà développée :
           ``value=5_000_000, evidence="cinq millions"`` -> ``5_000_000``

        L'ancienne implémentation multipliait également le deuxième cas par
        un million et produisait ``5_000_000_000_000``.
        """
        ...
        if multiplier > 1 and abs(numeric_value) < multiplier:
    ...
    def _normalize_candidate_value(
        ...
        if field == ParamName.total_file_count:
            total_file_count = self._normalize_total_file_count(
                value=value,
                unit=unit,
                evidence=evidence,
            )
    ...
    def extract(
        ...
        prompt = {
            ...
            "content": (
                ...
                "omit it.\n"
                "For total_file_count, return the final absolute integer in value "
                "(for example, five million -> 5000000) and use null as unit. "
                "Never return an expanded value together with a scale that should "
                "be applied again.\n"
                "Return JSON only.\n\n"
